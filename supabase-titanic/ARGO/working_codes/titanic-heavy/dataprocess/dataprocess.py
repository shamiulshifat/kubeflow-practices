...
def process_data(train:str, test:str):
    #read train data from supabase
    req_train = Request(train)
    req_train.add_header('User-Agent', 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:77.0) Gecko/20100101 Firefox/77.0')
    content_train = urlopen(req_train)
    ################
    train_data = pd.read_csv(content_train)
    print('original_training_dataset downloaded successfully')
    train_data.drop(['Name', 'Ticket', 'PassengerId', 'Cabin'], axis=1, inplace=True)
    print(train_data.shape)
    ###################
    req_test = Request(test)
    req_test.add_header('User-Agent', 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:77.0) Gecko/20100101 Firefox/77.0')
    content_test = urlopen(req_test)
    test_data = pd.read_csv(content_test)
    print('----------------------------')
    print('original_test_dataset downloaded successfully')
